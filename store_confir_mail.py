...
# Look for mail from cwj...
result, data = mail.search(None, '(FROM cwjobs@cwjobsmail.co.uk)')

# log file with current date and time
now = time.strftime("%c")
now = now.replace(' ', '_').replace('/', '-').replace(':', '-')
file = open("Store_confir_mail\Log_mail_stored_" + now + ".txt", "w")
...
for num in data[0].split():
    ...
    if subjd.strip().find('Application confirmation') != -1:
        subjd = subjd[26:]
        date = msg['Date']
        # consider only application after a certain date, first scripts
        # test 21/01/2016
        date = date[:11].strip()
        ad = date.split(' ')
        html = msg.get_payload()
        if datetime.date(int(ad[2]), month_string_to_number(ad[1]),
                         int(ad[0])) > datetime.date(2016, 1, 20):
            file.write("Mail " + str(num) +
                       " Date" + date +
                       " Subject: " + subjd + "\r\n")
            soup = BeautifulSoup(html, 'html.parser')
            # extracting from mail body of job id number and reference
            # person's mail
            for info in soup.findAll('a', href=True):
                # get job id
                if str(info['href']).find('JobId') != -1:
                    jid = info['href'][info['href'].find('JobId') + 6
                          :info['href'].find('&')]
                # get reference person's mail
                if info['href'].find('mailto') != -1 and \
                   info['href'].find('matteo.morini994') == -1:
                    contact_p = info['href'][7:]

            print("Application done on " + pulisci(date) +
                  " for:" + pulisci(subjd) +
                  " (job n." + pulisci(jid) + ")")
            file.write("Application done on " + pulisci(date) +
                       " for:" + pulisci(subjd) +
                       " (job n." + pulisci(jid) + ")\r")
            ...
            r = cur.fetchone()
            if r[0] == False:
                # upd main table with contact mail and changing
                # to true column applied
